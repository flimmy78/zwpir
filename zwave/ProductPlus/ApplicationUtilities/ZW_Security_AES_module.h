...
void ZCB_FuncZWSecure(BYTE txStatus);
...
//TRO: 09/09/2016    Nonce timer changed from 20 to 10. Only have 10 seconds timeout on NONCE job!
#define NONCE_TIMER                   10      /* Nonce Timer, min. 3 sec, rec.10, max 20 */
...
**
**-------------------------------------------------------------------------*/
void StartSecurityTimeoutIfNotRunning(BYTE timeout);
...
**    Called by ZW_TransportSecurity module to inform that an external nonce
**    has been received. If the ZW_Security_AES_Module needs the nonce, it
**    will proceed to transmit a secure payload.
**
**
**--------------------------------------------------------------------------*/
void NonceHasBeenReceived();

/* Called by FSM module when returning to idle state */
